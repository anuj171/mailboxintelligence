...
namespace dotnet_tutorial.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<ActionResult> Inbox()
        {
            ...
            try
            {
                var mailResults = await client.Me.MailFolders.Inbox.Messages.Request()
                                   // .OrderBy("receivedDateTime DESC")  -> Need to fix this to match with Filter query
                                    .Filter("from/emailAddress/address eq 'hack@microsoft.com'")
        ...
        public async Task<ActionResult> InboxWithQuery(string fromDay = "2018-07-16", string toDay = "2018-07-23", String containsStr = "", String fromEmail = "hack@microsoft.com")
        {
            string queryString = (String.IsNullOrEmpty(fromDay) ? "receivedDateTime ge 2000-01-01" : "receivedDateTime ge " + fromDay);
            queryString += (String.IsNullOrEmpty(toDay) ? "" : " and receivedDateTime lt " + toDay);
            queryString += (String.IsNullOrEmpty(containsStr) ? "" : " and subject eq '" + containsStr + "'");
            queryString += (String.IsNullOrEmpty(fromEmail) ? "" : " and from/emailAddress/address eq '" + fromEmail + "'");
